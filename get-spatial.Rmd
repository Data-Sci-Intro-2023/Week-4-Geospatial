...
Import the counties shapefile for Colorado again as you did in Week 1, along with linear water features for Larimer county (similar to how you imported roads, but check out the `linear_water()` function).
...
To filter rows that have a specific character string, you can use `str_detect()` within `filter()`. Fix the following code so that this operation works.
...
|                                        |                                                 |                                                             |
|:------------------:|:----------------------:|:--------------------------:|
| ![Elk](elk.jpg){alt="Elk" width="173"} | ![Marmot](marmot.jpg){alt="Marmot" width="173"} | ![Salamander](salamander.jpg){alt="Salamander" width="215"} |
|                  Elk                   |              Yellow-Bellied Marmot              |                  Western Tiger Salamander                   |

To pull occurrence data with this package you use the `occ_data()` function and give it a species name you want to retrieve data for. Since we want to perform this operation for three species, this is a good opportunity to work through iterative coding with for loops and the `purrr` package.
...
Challenge: Re-write the for loop to retrieve each species occurrences but using `purrr::map()` instead

Once you have your full data frame of occurrences for all three species, convert it to a spatial `sf` points object with crs set to 4326.
...
Exercise: Below is the string of operations you would use to import data for a single SNOTEL site for the year 2021. Use `purrr::map()` to pull data for all unique SNOTEL sites in the `snotel_sites` object you just created. Explore the use of the various `map()` functions to return a single data frame for all site data. Then as a final step, use `left_join()` to join the snow depth data to the station data to get the coordinates for all the sites, and make it a spatial object.
...
Exercise: save all the above vector objects you created above (counties, rivers, occurences, snotel, and watersheds) to a single .RData file in the data/ folder.
...
Exercise: Follow instructions from the Week 1 spatial lesson to import elevation data for Colorado at a zoom level of 7 and write it to a .tif file in the data/ folder of this repo. Make sure to crop the raster layer to the extent of Colorado, and give it the name "Elevation". Produce a quick plot to show your final raster object.
...
Read in the .tif file in the data/ folder of the repo. Make note of the auxillary file .aux.xml with the .tif file.
