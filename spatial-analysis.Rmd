...
1.  Project the landcover raster to the CRS of the counties shapefile

```{r}
landcover_prj <- terra::project(landcover, counties)
```

2.  Add a new column to `counties` that is the most common land cover type, using the function `modal` within `extract()`.

```{r}
counties$common_landcover <- terra::extract(landcover_prj, vect(counties), fun = "modal")[,2]
```

Notice however that this returns the raw raster values, which are not informative to use without knowing the associated landcover classes.
