...
Use `extract` from the `terra` package to create a new column in the `occ` dataset that includes the elevation for each occurrence. Then edit the interactive map you just made above but add 'Elevation' to the popup window.

```{r}
occ$elevation <- extract(elevation, occ)[,2]
...
visitation <-
  unit_visitation(
    units = c("DEVA", "JOTR", "MOJA", "BRCA", "ZION"),
    startYear = 1980,
    endYear = 2021
  )


park_boundaries <- getParkBoundary(park = c("DEVA", "JOTR", "MOJA", "BRCA", "ZION"))
```

```{r}
annual_visitation <- visitation %>% 
  group_by(UnitCode, Year) %>% 
  summarise(total=sum(RecreationVisitors)) 

plot_ly(annual_vistiation) %>% 
  add_trace(x = annual_visitation$Year,
            y = annual_visitation$total,
            name = ~annual_visitation$UnitCode,
            linetype = ~annual_visitation$UnitCode,
            type = "scatter",
            mode = "lines+markers") %>% 
  layout(yaxis = list(title = "Total Annual Recreation Visitors"))
```
...
annual_visitation %>%
  filter(Year == 2021) %>% 
  left_join(park_boundaries, by = c("UnitCode" = "UNIT_CODE")) %>% 
  st_as_sf() %>% 
  qtm(fill = "total")
